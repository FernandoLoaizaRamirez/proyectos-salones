 ...
 * SERVIDOR: si la función "dinamicas" no está habilitada para el evento, el
 * módulo ni siquiera se renderiza (esconder la tarjeta en la home no basta).
 */
...
export default async function Page({
  ...
  const habilitado = tieneFuncion(config.entitlements, F.Dinamicas);
  ...
  return (
    <BrandingScope branding={branding} className="min-h-screen">
      <main className="mx-auto max-w-2xl px-6 py-10">
        ...
        <h1 className="mt-4 text-3xl font-semibold tracking-tight">Dinámicas y juegos</h1>
        <p className="mt-2 mb-8 text-muted-foreground">{config.nombre}</p>

        {habilitado ? (
          <DinamicasModulo evento={codigo} nombreEvento={config.nombre} />
        ) : (
          <Card className="flex items-start gap-4 p-6">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Las dinámicas y juegos no están incluidas en este evento.
              </p>
            </div>
          </Card>
        )}
