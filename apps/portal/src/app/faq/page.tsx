/**
 * Ruta del MÓDULO PREGUNTAS FRECUENTES dentro del portal (Server Component).
 *
...
import { FaqModulo } from "@/modulos/info/faq-modulo";

export function generateMetadata({ searchParams }: { searchParams: Promise<{ e?: string }> }) {
  return metadataModulo(searchParams, "Preguntas frecuentes");
}
...
export default async function Page({
  ...
  const ctx = await contextoModulo(searchParams, F.Faq);
  ...
  return (
    <AppShell tema={ctx.config.tema} ancho="3xl" {...ctx.navegacion}>
      ...
      {ctx.habilitado ? (
        <FaqModulo evento={ctx.codigo} />
      ) : (
        <ExperienciaNoIncluida que="La sección de preguntas" />
      )}
    </AppShell>
  );
}
