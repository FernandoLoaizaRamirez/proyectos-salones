...
export function generateMetadata({ searchParams }: { searchParams: Promise<{ e?: string }> }) {
  return metadataModulo(searchParams, "Muro de mensajes");
}
...
export default async function Page({
  ...
  const ctx = await contextoModulo(searchParams, F.Muro);
  ...
  return (
    <AppShell tema={ctx.config.tema} ancho="3xl" {...ctx.navegacion}>
      ...
      {ctx.habilitado ? (
        <MuroModulo evento={ctx.codigo} nombreEvento={ctx.config.nombre} />
      ) : (
        <ExperienciaNoIncluida que="El muro de mensajes" />
      )}
    </AppShell>
  );
}
