 ...
 * SERVIDOR: si la función "rsvp" no está habilitada para el evento, el módulo
...
export default async function Page({
  ...
  const habilitado = tieneFuncion(config.entitlements, F.Rsvp);
  ...
  return (
    <BrandingScope branding={branding} className="min-h-screen">
      <main className="mx-auto max-w-lg px-6 py-10">
        ...
        <h1 className="mt-4 text-3xl font-semibold tracking-tight">Confirmar asistencia</h1>
        <p className="mt-2 mb-8 text-muted-foreground">{config.nombre}</p>

        {habilitado ? (
          <RsvpModulo evento={codigo} nombreEvento={config.nombre} />
        ) : (
          <Card className="flex items-start gap-4 p-6">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                La confirmación de asistencia no está incluida en este evento.
              </p>
            </div>
          </Card>
        )}
