...
| `0032_candados_etapa3.sql` | **Los dos candados que la revisión adversaria le encontró a la Etapa 3**: FK compuesta `events(client_id, tenant_id) → clients(id, tenant_id)` con `on delete set null (client_id)` (PG 15+; sin la lista de columnas, borrar una ficha tronaría el NOT NULL de tenant_id) — un evento ya NO puede señalar al cliente de OTRO salón (las FK se saltan la RLS); y `actividad.evento → events(codigo)` con cascada (borrar/renombrar el evento se lleva sus contadores; se limpiaron huérfanos antes de atar). | ✅ **Corrida el 28 ago 2026 y verificada con ataque real**: cruzar la boda de muestra con el cliente de la Hacienda → 23503; ligar el propio → 204. |
| `0031_actividad.sql` | **El primer registro de actividad** (sección 17 del Prompt Maestro, versión mínima): tabla `actividad` (evento, tipo, día, contador — SIN identidad, la línea de la 0012) + `apuntar_actividad` (SECURITY DEFINER pública: valida allowlist portal/invitacion/rsvp/pase, formato y que el evento exista activo — las vitrinas se ignoran solas). Lectura solo para el staff del salón dueño; escritura SOLO por la función. | ✅ **Corrida el 27 ago 2026 y verificada en vivo**: dos llamadas → contador 2; evento inventado ignorado; INSERT directo con anon → 42501; la cuenta de muestra lee solo lo suyo. Inflable a mano a propósito (documentado): el daño posible es un número presumido, jamás datos. |
| `0030_clientes.sql` | **La ficha de clientes del salón** (el CRM mínimo): tabla `clients` (nombre, teléfono, correo, notas) acotada por salón + `events.client_id` con `set null` (borrar la ficha no borra su boda). Escribe cualquier miembro (trabajo de mostrador). Siembra los clientes de la caja de arena (Carmen & Luis) y de la demo (Ana & Rodrigo). | ✅ **Corrida el 27 ago 2026** y verificada (semillas ligadas). ⚠️ Su FK a secas dejaba cruzar salones — la cierra la `0032`. |
| `0029_cuenta_de_muestra.sql` | **La cuenta de muestra**: nace el "Salón de Muestra" (tenant `aa…01`, plan gestionado, marca casi en blanco a propósito), su boda semilla (`boda-de-muestra`, Carmen & Luis) y el usuario `muestra@suite-salones.app` (dueño SOLO de esa caja de arena — el aislamiento de la 0008 lo encierra). Credenciales públicas por diseño (salen en el catálogo). **Re-correrla = botón de reset** (marca, boda, contraseña y claims vuelven al original). ⚠️ Escribe en el esquema `auth` (patrón estándar de siembra GoTrue). | ✅ **Corrida el 27 ago 2026 y verificada con LOGIN REAL contra producción**: el token trae los claims, ve SOLO su evento, rayar la marca de la Hacienda afecta 0 filas, leer el event_branding ajeno da vacío, y vestir su propio salón funciona. |
